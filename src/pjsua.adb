...
package body PJSUA is
   ...
   begin
      if Integer(C_Pjsua_Transport_Create (C.Int(Transport_Type),
				   ...
				   C.Strings.Null_Ptr)) /= PJ_Types.Success then
	 raise PROGRAM_ERROR;
      end if;
   ...
   function To_Pj_String(Item : in String) return PJ_Types.String_T is
      function C_PJ_Str (Str : C.Strings.Chars_Ptr) return PJ_Types.String_T;
      pragma Import (C,C_PJ_Str,"pj_str");
      Str : PJ_Types.String_T;
   begin
      Str := C_PJ_Str(C.Strings.New_String(Item));
      Put_Line("To_PJ_String: Ptr => " & C.Strings.value (Str.Ptr) & 
		 " Size => " & PJ_Types.Size_T'Image(Str.Size));
      return Str;
   ...
   begin
      if Integer ((C_Pjsua_Init
	    (Config_Ptr,Logging_Config_Ptr, C.Strings.Null_Ptr))) /= PJ_Types.Success then
	 raise PROGRAM_ERROR;
      end if;
   end Init;
